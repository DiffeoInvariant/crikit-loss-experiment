...
def run_experiment(seed, std, loss):
    ...
    fe_order = 3 # finite element order; must be >= 2
    ...
    eps2 = epsilon ** 2
    ex = Experiment(amesh, alpha=angle_rads, g=g)#angle_radians=angle_rads, g=g, fe_order=fe_order)
    ...
    plt.title(f"Noise-free observations with a = 1.0, p = 1.2")
    plt.show()
